...
fun Negocio.toDomainModel(reviews: List<Resena> = emptyList()): Business {
    ...
    if (reviews.isNotEmpty()) {
        avgAtencion = reviews.map { it.calidadAtencion }.average()
        avgProducto = reviews.map { it.calidadProductos }.average()
        avgCosto = reviews.map { it.costos }.average()
    }
